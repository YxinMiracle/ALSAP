...
class CRFTAGGER(nn.Module):
    def __init__(self, params, vocab):
        ...
        self.embedding = nn.Embedding(vocab.n_words, params.lstm_hidden_dim*2, padding_idx=0)
        embedding = load_embedding(vocab, params.lstm_hidden_dim*2, params.emb_file, params.usechar)
        self.embedding.weight.data.copy_(torch.FloatTensor(embedding))

        self.dropout = params.dropout
        self.lstm = nn.LSTM(params.emb_dim, params.lstm_hidden_dim, num_layers=params.n_layer, dropout=params.dropout,
    ...
    def forward(self, X, return_hiddens=False):
        ...
        lstm_hidden = F.dropout(embeddings, p=self.dropout, training=self.training)
        # lstm_hidden, (_,_) = self.lstm(embeddings)  # (bsz, seq_len, 2*hidden_dim)
