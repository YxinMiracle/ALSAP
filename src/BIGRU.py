...
class BiGruTagger(nn.Module):
    def __init__(self, params, vocab):
        ...
        self.gru = nn.GRU(params.emb_dim, params.gru_hidden_dim, batch_first=True,bidirectional=True)
        self.linear = nn.Linear(params.gru_hidden_dim * 2, params.num_tag)
    ...
    def forward(self, X, return_hidden=False):
        ...
        """
        embeddings = self.embedding(X)  # shape is (bsz, seq_len, emb_dim)
        gru_output, (_) = self.gru(embeddings)
        prediction = self.linear(gru_output)
        return prediction
