...
class BiLSTMTagger(nn.Module):
    def __init__(self, params, vocab):
        ...
        # emb_dim = 300 对应与glove.6B.300d文件，其中这里的padding_idx，是指定要是数值为0的话，那么对应的词向量就是0
        ...
        self.first_lstm = nn.LSTM(params.emb_dim, params.lstm_hidden_dim, num_layers=params.n_layer,
                                  dropout=params.dropout, batch_first=True)
        self.lstm = nn.LSTM(params.lstm_hidden_dim, params.lstm_hidden_dim, num_layers=params.n_layer, dropout=params.dropout,
    ...
    def forward(self, X, return_hiddens=False):
        ...
        lstm_hidden, (_, _) = self.first_lstm(embeddings)  # (bsz, seq_len, 2*hidden_dim)
        lstm_hidden, (_, _) = self.lstm(lstm_hidden)  # (bsz, seq_len, 2*hidden_dim)
